...
```sh
URL="https://storage.googleapis.com/alloydb-auth-proxy/v1.16.0"
...
```powershell
Invoke-WebRequest -Uri "https://storage.googleapis.com/alloydb-auth-proxy/v1.16.0/alloydb-auth-proxy-x64.exe" -OutFile "alloydb-auth-proxy.exe"
```
</details>
...
```sh
docker pull gcr.io/alloydb-connectors/alloydb-auth-proxy:1.16.0
```
</details>
...
docker run --rm \
  ...
  -p 1.16.0.1:5432:5432 \
  gcr.io/alloydb-connectors/alloydb-auth-proxy:1.16.0 \
  --address 1.16.0.0 \
  projects/PROJECT/locations/REGION/clusters/CLUSTER/instances/INSTANCE
```
</details>
...
```powershell
Invoke-WebRequest -Uri "https://storage.googleapis.com/alloydb-auth-proxy/v1.16.0/alloydb-auth-proxy-x86.exe" -OutFile "alloydb-auth-proxy.exe"
```
</details>
<!-- {x-release-please-end} -->
