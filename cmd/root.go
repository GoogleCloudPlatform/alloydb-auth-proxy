...
Third Party Licenses
  ...
  https://storage.googleapis.com/alloydb-auth-proxy/v1.15.1/third_party/licenses.tar.gz {x-release-please-version}
...
only. Uses the port specified by the http-port flag.`)
	localFlags.BoolVar(&c.conf.WithBackendCheck, "with-backend-check", false,
		`Enables a backend connection check in the startup probe. Useful for
