...
const createPool = async () => {
  ...
  config.pool.min = 5;
  // [END alloydb_knex_limit]

  // [START alloydb_knex_timeout]
  ...
  const {ALLOYDB_CREDENTIALS_SECRET} = process.env;
  if (ALLOYDB_CREDENTIALS_SECRET) {
    const secrets = await accessSecretVersion(ALLOYDB_CREDENTIALS_SECRET);
    try {
